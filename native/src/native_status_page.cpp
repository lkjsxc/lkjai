...
function applyModel(){const can=!pending;$('message').disabled=!can;$('send').disabled=!can;$('chatState').textContent=pending?'sending':reason()||'chat ready';$('modelLine').textContent=`${model.model||'model'} / ${model.artifact_kind||'unknown'} / ${config.tool_profile||model.tool_profile||'tool profile pending'}`;$('status').textContent=`${health.status||'unknown'} / ${model.loaded?'loaded':'not loaded'}`}
function startDraft(){viewToken++;runId='';events=[];$('message').value='';$('title').textContent='Draft run';$('runState').textContent='No persisted runs';renderEvents();applyModel();loadRuns('')}
