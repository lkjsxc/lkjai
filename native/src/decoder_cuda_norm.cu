...
__global__ void rmsnorm_backward_f32_dout_kernel(
    const __nv_bfloat16* input, const float* weight, const float* d_output,
    float* d_input, float* d_weight, int rows, int hidden, float eps) {
  extern __shared__ float scratch[];
  int row = blockIdx.x;
  if (row >= rows) return;
  const auto* x = input + static_cast<size_t>(row) * hidden;
...
void decoder_launch_rmsnorm_backward_bf16_f32_dout(
  ...
  if (d_weight_beta == 0.0f) {
    require_cuda(cudaMemsetAsync(d_weight_f32, 0,
                                 static_cast<size_t>(hidden) * sizeof(float),
                                 stream),
                 "decoder rmsnorm f32 dout d_weight zero");
  } else if (d_weight_beta != 1.0f) {
    throw std::runtime_error(
        "decoder RMSNorm f32 dout backward supports beta 0 or 1");
  }
  rmsnorm_backward_f32_dout_kernel<<<rows, 256, 256 * sizeof(float), stream>>>(
      static_cast<const __nv_bfloat16*>(input_bf16), weight_f32, d_output_f32,
      d_input_f32, d_weight_f32, rows, hidden, eps);
  require_cuda(cudaGetLastError(),
               "decoder_rmsnorm_backward_f32_dout_kernel");
}
