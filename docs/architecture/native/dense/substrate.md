...
## BF16 Optimization Contract

The accepted dense path uses cuBLASLt for the LM-head gradient GEMM
`grad_head += grad_logits^T * hidden` and hidden-gradient GEMM
`d_hidden = grad_logits * lm_head`. Token embedding gradients stay FP32 and use
token scatter-add accumulation so gradient accumulation matches the original
dense trainer contract.

Dense reports may add backend and memory fields for cuBLASLt workspace, loss
readback, logits readback, stream count, and batch-slot count, but those fields
are additive. They do not promote transformer CUDA, chat decode, CUDA Graph
capture, NCCL, or alternate precision modes.

Parity tolerance may be relaxed only for documented cuBLASLt accumulation-order
drift, and only up to `5e-3`. Larger drift blocks promotion.
