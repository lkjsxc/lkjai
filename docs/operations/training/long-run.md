...
- `TRAIN_MODEL_PRESET`: default `scratch-20m`
...
- `TRAIN_BATCH_POLICY`: `fixed`, `oom_fallback`, or `sweep`; default `fixed`
...
- `TRAIN_SAVE_LATEST_EVERY_OPTIMIZER_STEPS`: default `250` for non-quick,
  `1` for quick
- `TRAIN_INTERMEDIATE_SAVE_EVERY_OPTIMIZER_STEPS`: default `1000` for
  non-quick, `0` for quick
- `TRAIN_KEEP_LAST_CHECKPOINTS`: default `3`
