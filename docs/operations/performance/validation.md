...
- `native_decoder_tokenizer_contract` uses a generated byte-level BPE fixture,
  so verification does not depend on ignored `data/train/tokenizer` files.
- `native_decoder_artifact_contract` fails if decoder artifacts write
  `pos_embeddings`, tokenizer checksums are wrong, inspect/logits checks fail,
  or partial CUDA report fields over-claim support.
- `native_decoder_cuda_block_forward_substrate`,
  `native_decoder_cuda_rmsnorm_parity`, `native_device_tensor_check`,
  packed-cache checks, runtime contracts, and smoke export cover the current
