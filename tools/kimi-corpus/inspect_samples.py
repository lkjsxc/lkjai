...
def main() -> None:
    ...
    parser.add_argument("--prompt-refiner", default="tools/kimi-corpus/prompts/prompt_refiner.txt")
...
def refine_prompts(report: str, refiner_path: Path) -> None:
    ...
    Path("tools/kimi-corpus/prompts/prompt_refiner_candidate.txt").write_text(result.stdout_path.read_text(encoding="utf-8", errors="replace"), encoding="utf-8")
